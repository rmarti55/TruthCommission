...
import { ingestSubpoenas } from "@truth-commission/ingest";
import { finalizeIngestedSubpoenas } from "../src/lib/process-subpoena";
...
async function main() {
  ...
  const result = await ingestSubpoenas(db);

  console.log("Seed ingest complete:");
  console.log(`  ingested: ${result.ingested.length}`);
  console.log(`  skipped: ${result.skipped.length}`);
  console.log(`  errors: ${result.errors.length}`);

  if (result.ingested.length > 0) {
    const finalized = await finalizeIngestedSubpoenas(db, result.ingested);
